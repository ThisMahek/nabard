...
class State extends CI_Controller
{
    ...
    public function  getch_state_district_block(){

        $state_id = isset($_GET['state']) ? $_GET['state'] : 0;
    //    $get_district = $this->get_state_district($district_id=='');
        $get_district = $this->db->get_where('district',['state_id'=>$state_id,'status'=>1])->result_array();
        if (isset($state_id)) {
          $collectDistrict = [];
          if (!empty($get_district)) {
            ...
            $collectDistrict[] = '<option>--Select District--</option>';
          }
          echo json_encode(['subcategory_list' => $collectDistrict, 'demo' => '1']);
        } else {
          $collectDistrict[] = '<option>--Select District First--</option>';
          echo json_encode(['city_list' => $collectDistrict, 'demo' => 'else']);
        }
      ...
      public function   getch_state_district_disabled()
      {

        $state_id = isset($_GET['state'])?$_GET['state'] : 0;
    //    $get_district = $this->get_state_district($district_id=='');
        $get_district = $this->db->get_where('district',['state_id'=>$state_id,'status'=>0])->result_array();
        if (isset($state_id)) {
          $collectDistrict = [];
          if (!empty($get_district)) {
            $collectDistrict[] = '<option value="">--Select District--</option>';
          ...
          $collectDistrict[] = '<option>Select District First</option>';
          echo json_encode(['subcategory_list' => $collectDistrict, 'demo' => 'else']);
        }
...
/* public function update_status(){
    $this->db->where(['status'=>1])->update('tbl_state',['status'=>0]);
    $this->db->where(['status'=>1])->update('district',['status'=>0]);
} */

     public function  getch_state_district_enabled()
     {
    ...
    //    $get_district = $this->get_state_district($district_id=='');
      ...
      public function change_district_status($id)
      {
       ...
       $status = $this->db->query("select status from district where id =$id")->row()->status;
		if ($status == 1) {
			$productstatus = 2;
			$this->session->set_flashdata('success', ' <div class="alert alert-success alert-dismissible fade show">District Disabled Successfully<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>');
		} else {
			$productstatus = 1;
    ...
    public function  change_block_status($id)
      {
        $status = $this->db->query("select status from block where id =$id")->row()->status;
         if ($status == 1) {
             $productstatus = 2;
             $this->session->set_flashdata('success', ' <div class="alert alert-success alert-dismissible fade show"> Block Disabled Successfully<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>');
         } else {
             $productstatus = 1;
             $this->session->set_flashdata('success', ' <div class="alert alert-success alert-dismissible fade show">Block Enabled Successfully <button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>');
         }
         $data = array('status' => $productstatus);
         $this->db->where('id', $id);
      ...
      public function  update_block(){
       ...
       $dname = $data['district'];
       $sname = $data['state'];
        $res = $this->db->where(['id'=>$b_id])->update('block',['name'=>$bname,'district_id'=>$dname]);
        if ($res == 1) {  
            $this->session->set_flashdata('success', ' <div class="alert alert-success alert-dismissible fade show"> Block Updated Successfully<button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>');
        } else {
            $this->session->set_flashdata('success', ' <div class="alert alert-danger alert-dismissible fade show">Block Unable to Update  <button type="button" class="btn-close" data-bs-dismiss="alert"></button></div>');
        }
        redirect(base_url()."admin/manage_block");
      }
